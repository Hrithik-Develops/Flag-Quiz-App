package com.singh.flagquizapp.database
...
class FlagsDao {
    ...
    fun getRandomTenRecords(helper: DatabaseCopyHelper): ArrayList<FlagsModel>
    {
        ...
        val flagIdIndex=cursor.getColumnIndex("flag_id")
        val countryNameIndex=cursor.getColumnIndex("country_name")
        val flagNameIndex=cursor.getColumnIndex("flag_name")

        while (cursor.moveToNext()){
            val record=FlagsModel(cursor.getInt(flagIdIndex),cursor.getString(countryNameIndex),cursor.getString(flagNameIndex))
